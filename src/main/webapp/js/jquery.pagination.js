jQuery.fn.pagination = function (maxentries, opts) {
    opts = jQuery.extend({
        items_per_page: 50, // 每页显示多少条记录
        current_page: 0,      //当前页码
        num_display_entries: 5, // 中间显示页码的个数
        num_edge_entries: 5, // 末尾显示页码的个数
    ...
    return this.each(function () {
        ...
        function drawLinks() {
            ...
            if (opts.display_msg) {
                if (!maxentries) {
                    panel
                        .append('<div class="pxofy">暂无数据可以显示</div>');
                } else {
                    panel
        ...
        if (maxentries == 0) {
            // panel.append('<span class="current prev">' + opts.prev_text + '</span><span class="current next">' + opts.next_text + '</span><div class="pxofy">暂无数据可以显示</div>');
            panel.html('<div class="pxofy">暂无数据可以显示</div>');
        } else {
            drawLinks();
        }
